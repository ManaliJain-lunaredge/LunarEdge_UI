...
const Header = ({ toggletheme, theme }) => {
  ...
  return (
<div className="fixed top-0 left-0 w-full z-50 " style={{backgroundImage:`url(${header_bg})`}}>
<div className="container mx-auto ">
      <div className={`flex justify-between items-center font-poppins pt-[1rem] p-[1rem] md:pt-[1rem] md:p-[1rem] lg:pt-[1rem] lg:p-[1rem] xl:pt-[1rem]   ${
        scrolled ? "2xl:pt-[1rem] 2xl:pb-[1rem] 3xl:px-[6rem] lg:px-[2rem]" : "2xl:pt-[2rem] 2xl:py-[3rem] 2xl:px-[3.5rem] "} `}>
        {/* Logo Section */}
          ...
          <ul className="flex justify-between gap-[5rem] 2xl:gap-[5rem]  mt-[1rem] lg:gap-[3rem] pl-[7rem] 2xl:pl-[7rem] lg:pl-0 ">
            {navbar.map((item) => (
              <li
                key={item.id}
                className={`${
                  active === item.to ? "text-[#FB2B81]" : "text-[#ffffff]"
                } hover:text-[#FB2B81] cursor-pointer font-medium text-lg 2xl:text-xl lg:text-sm relative`}
        ...
        <div className="fixed bg-[#190F21] top-0 left-0 w-[100%] md:w-[100%] h-full z-50">
          ...
          <ul className="flex flex-col text-left gap-5 items-start text-white font-medium text-[18px] px-4 py-6">
            {navbar.map((item) => (
             <li
             key={item.id}
             className={`relative ${active === item.to ? "text-[#FB2B81]" : "text-white"} 
               hover:text-[#FB2B81] cursor-pointer font-medium text-lg`}
             onMouseEnter={() => handleMouseEnter(item.name.toLowerCase())}
             onMouseLeave={handleMouseLeave}
             onClick={() => toggleDropdown(item.name.toLowerCase())} // Enable click for mobile
           >
             <Link to={item.to} className="flex items-center">
               {item.name}
               {item.subItems && <RiArrowDropDownLine className="text-3xl" />}
             </Link>
             {item.subItems && activeDropdown === item.name.toLowerCase() && (
               <ul className="absolute left-[8rem] w-[212px] bg-white text-black rounded-md shadow-lg">
                 {item.subItems.map((subItem, index) => (
